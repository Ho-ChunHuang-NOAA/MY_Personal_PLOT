...
#==================================================================================================
# USAGE make-fvs_all.sh  plottype
# plottype :
...
#   SET PLOT TITLE
    ...
    if [ $plottype = DIU ]; then 
      ...
      if [ $vbrt = "TO" ]; then
        ...
        title="Surface $varb over $rgnd from $strdatet to $endatet"
      fi
    ...
    if [ $plottype = FHO ];then 
      ...
      if [ $vbrt = "TO" ]; then
        ...
        xlabel="24-h precipitation in inches (all cycles)"
      fi
    fi
