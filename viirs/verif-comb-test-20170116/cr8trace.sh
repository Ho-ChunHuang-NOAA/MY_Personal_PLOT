...
# Bin by plot level for 1 forecast hour
    ...
    set -A blvls P850 P700 P500 P400 P300 P250 P200 P150
    if [ $VARB = RH ];then
      set -A blvls P850 P700 P500 P400 P300 
    fi
    nlvls=${#blvls[@]}
...
for TRACE in ${traces[*]};do
  let itrace=itrace+1
  if [ $itrace -eq 1 -a $sttyp != FHO ];then 
...
#   Right now, the program will not categorize by verification hour when a range is used
    ...
    echo $itrace "15 0" $conchki >>trace.ctl 
