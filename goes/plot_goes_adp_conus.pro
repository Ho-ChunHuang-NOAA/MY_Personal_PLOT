...
@/u/Ho-Chun.Huang/IDL/my.idllib/satellite_colorscalei_rainbow_no_purple.pro
...
function read_abi_var,filename,var_name, scaled
   ...
   if flag_check then begin
      ...
      adpmax=max(pretemp,ilocmax)
      adpmin=min(pretemp,ilocmin)
      print, var_name+' Valid values are maximum = '+string(adpmax,'(f12.4)')+'   minimum = '+string(adpmin,'(f12.4)')
         
      indexT=where(pretemp gt 0., nidx)
      if nidx gt 0L then begin
         print, var_name+' number > 0 is '+string(nidx,'(i10)')
         adp_test=pretemp(indexT)
         adpmax=max(pretemp(indexT),ilocmax)
         adpmin=min(pretemp(indexT),ilocmin)
         print, var_name+' maximum = '+string(adpmax,'(f12.4)')+'   minimum = '+string(adpmin,'(f12.4)')
      endif
      ...
      if nidx gt 0L then begin
         ...
         adpmax=max(pretemp(indexT),ilocmax)
         adpmin=min(pretemp(indexT),ilocmin)
         print, 'maximum = '+string(adpmax,'(f12.4)')+'   minimum = '+string(adpmin,'(f12.4)')
      endif
   endif
...
pro plot_goes_adp_conus
   @run.plot_goes_adp_conus.h
   print, 'Program plot_goes_adp_conus.pro START AT '+SYSTIME()
   idum1 = -999L
   ;;
   ...
   lat2km=fltarr(nx,ny)
   lon2km=fltarr(nx,ny)
   llfname='/gpfs/dell2/emc/modeling/noscrub/Ho-Chun.Huang/GOES16_AOD/g16_conus_latlon_2km_20180620.dat'
   ...
   FOR ifile = 0L, nfile-1L DO BEGIN
      ...
      ;; ' /gpfs/${phase12_id}d3/emc/meso/noscrub/Ho-Chun.Huang/GOES16_AOD/ADP/DR_ABI-L2-ADPC-M3_G16_s20180211802203_e20180211804576_c20180211807552.nc'
      IF ~ File_Test(fname) THEN BEGIN
         print, 'Can not find '+fname
         continue
      ENDIF
      print, fname
      tmp = strsplit(adp_files[ifile],'_',/extract,count=cnt)
      ...
      dust = read_abi_var(fname,'Dust',0)
      smoke = read_abi_var(fname,'Smoke',0)
      DQF=read_abi_var(fname,'DQF',0)

      smoke_con=smoke
      dust_con=dust
      ;; smoke_con=uintarr(nx,ny)
      ;; dust_con=uintarr(nx,ny)
...
;;
;; the output will switch the order so the qc values is in consistent with AOD qc values, i.e., 0 is high, 1 is medium, and 2 is low
      tmp_mask=DQF
...
;Byte1, bit7 (solar and satellite angles)
   ...
   index=where(smoke_con ge 1,  nidx)
   uadpqf=uint(adpqf)
   print, 'uadpqf= ', uadpqf
   print, 'UINT ADPQF = '+string(uadpqf,'(i1)')
   IF nidx NE 0L THEN BEGIN
      g16smoke=smoke_con(index)
      g16smokelat=lat2km(index)
      g16smokelon=lon2km(index)
      ; print,'g16 smoke mask qf is ',
      strConfidence=' '
      strfile=' '
      if adpqf eq 0L then begin
         index=where(g16smoke eq 3, nidx)
         strConfidence='High Confidence=3'
         strfile='high'
         print,'number of Smoke AOD QC <='+string(adpqf,'(i1)')+' is '+string(nidx,'(i10)')
      endif else begin
         if adpqf eq 1L then begin
            index=where(g16smoke ge 2, nidx)
            strConfidence='Medium Confidence=2 or 3'
            strfile='medium'
            print,'number of Smoke AOD QC <='+string(adpqf,'(i1)')+' is '+string(nidx,'(i10)')
         endif else begin
            index=where(g16smoke ge 1, nidx)
            strConfidence='Low Confidence=1 or 2 or 3'
            strfile='low'
            print,'number of Smoke AOD QC <='+string(adpqf,'(i1)')+' is '+string(nidx,'(i10)')
         endelse
      endelse
      g16adp=g16smoke(index)
      g16lat=g16smokelat(index)
      g16lon=g16smokelon(index)
      flag_check=0B
      if flag_check then begin
         adpmax=max(g16adp,ilocmax)
         adpmin=min(g16adp,ilocmin)
         print, 'Valid values are maximum = '+string(adpmax,'(f10.4)')+'   minimum = '+string(adpmin,'(f10.4)')
         
         indexT=where(g16adp gt 0, nidx)
         if nidx gt 0L then begin
            print, 'number of Smoke > 0 is '+string(nidx,'(i10)')
            adp_test=g16adp(indexT)
            adpmax=max(g16adp(indexT),ilocmax)
            adpmin=min(g16adp(indexT),ilocmin)
            print, 'maximum = '+string(adpmax,'(f10.4)')+'   minimum = '+string(adpmin,'(f10.4)')
         endif
         
         indexT=where(g16adp eq 0, nidx)
         if nidx gt 0L then begin
            print, 'number of Smoke mask = 0 is '+string(nidx,'(i10)')
         endif
         
         indexT=where(g16adp lt 0, nidx)
         if nidx gt 0L then begin
            print, 'number of Smoke mask < 0 is '+string(nidx,'(i10)')
            adp_test=g16adp(indexT)
            adpmax=max(g16adp(indexT),ilocmax)
   ...
   ;; only keep adp values with in domain of latlonglimits
   ;; latlonglimits = [15., -150. ,60., -50.]         ;; lat0, lon0, lat1, lon1
   ...
   figTitle = strYYYYMMDD+' '+strGMT+'Z'+' ADP Smoke Mask'+'   '+strConfidence
   xyouts, 0.5,ImagePosition[3L]+0.025, figTitle, /normal, alignment=0.5, charsize=title_size, font=1, color=ipen
               ...
               newimage( p( 0, * ) * ( image_xsize - 1 ), p( 1, * ) * ( image_ysize - 1 ) ) = $
                         bytscl( g16adp, min = float(range[0]), max = float(range[1]), top = !d.table_size - 5 ) + 1B
               image = float(g16adp)
              ...
              aot_plot=odir+'g16.adp.smoke.mask.'+string(syear,'(i4.4)')+string(xmon,'(i2.2)')+string(xday,'(i2.2)')+'_'+string(start_hr,'(i2.2)')+'.'+strfile+'.png'
                  WRITE_PNG, aot_plot, newimage,r,g,b
   ENDIF ELSE print, 'no valid ADP smoke mask values was found'
;; plot dust mask
   index=where(dust_con ge 1,  nidx)
   IF nidx NE 0L THEN BEGIN
      g16dust=dust_con(index)
      g16dustlat=lat2km(index)
      g16dustlon=lon2km(index)
      ; print,'g16 dust mask qf is ',
      strConfidence=' '
      strfile=' '
      if adpqf eq 0L then begin
         index=where(g16dust eq 3, nidx)
         strConfidence='High Confidence=3'
         strfile='high'
         print,'number of Dust  AOD QC <='+string(adpqf,'(i1)')+' is '+string(nidx,'(i10)')
      endif else begin
         if adpqf eq 1L then begin
            index=where(g16dust ge 2, nidx)
            strConfidence='Medium Confidence=2 or 3'
            strfile='medium'
            print,'number of Dust  AOD QC <='+string(adpqf,'(i1)')+' is '+string(nidx,'(i10)')
         endif else begin
            index=where(g16dust ge 1, nidx)
            strConfidence='Low Confidence=1 or 2 or 3'
            strfile='low'
            print,'number of Dust  AOD QC <='+string(adpqf,'(i1)')+' is '+string(nidx,'(i10)')
         endelse
      endelse
      g16adp=g16dust(index)
      g16lat=g16dustlat(index)
      g16lon=g16dustlon(index)
      flag_check=0B
      if flag_check then begin
         adpmax=max(g16adp,ilocmax)
         adpmin=min(g16adp,ilocmin)
         ...
         indexT=where(g16adp gt 0., nidx)
         if nidx gt 0L then begin
            print, 'number of Dust > 0 is '+string(nidx,'(i10)')
            adp_test=g16adp(indexT)
            adpmax=max(g16adp(indexT),ilocmax)
            adpmin=min(g16adp(indexT),ilocmin)
            print, 'maximum = '+string(adpmax,'(f10.4)')+'   minimum = '+string(adpmin,'(f10.4)')
         ...
         indexT=where(g16adp eq 0., nidx)
         if nidx gt 0L then begin
            print, 'number of Dust mask = 0 is '+string(nidx,'(i10)')
         endif
   ...
   map_set, 0., 0., limit=latlonglimits, /Cylindrical, color=ipen
   MAP_CONTINENTS, /CONTINENTS, /USA, /countries, /HIRES, color=iline
   figTitle = strYYYYMMDD+' '+strGMT+'Z'+' ADP Dust Mask'+'   '+strConfidence
   xyouts, 0.5,ImagePosition[3L]+0.025, figTitle, /normal, alignment=0.5, charsize=title_size, font=1, color=ipen
              ...
              aot_plot=odir+'g16.adp.dust.mask.'+string(syear,'(i4.4)')+string(xmon,'(i2.2)')+string(xday,'(i2.2)')+'_'+string(start_hr,'(i2.2)')+'.'+strfile+'.png'
                  WRITE_PNG, aot_plot, newimage,r,g,b
   ENDIF ELSE print, 'no valid ADP dust  mask values was found'
                  ;; Close device AND return to system device
                  DEVICE, /CLOSE
   ENDFOR
   print, 'Successfully finish.  Exit plot_goes_adp_conus.pro AT '+SYSTIME()
end
