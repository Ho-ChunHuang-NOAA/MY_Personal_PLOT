...
aod_type=dustaod
aod_file_type=`echo ${aod_type} | tr '[:lower:]' '[:upper:]'`
ntyp=${#type[@]}

ftpdir=/lfs/h2/emc/stmp/${USER}/daily_plot_${mdlname}_${aod_type}
mkdir -p ${ftpdir}
...
while [ ${NOW} -le ${LASTDAY} ]; do
   ...
   outdir=${ftpdir}/${obssat}_${aod_type}_${mdlname}_${NOW}
   ...
   tmpdir=/lfs/h2/emc/stmp/${USER}/com2_${aod_type}_${obssat}_${mdlname}.${NOW}
   if [ -d ${tmpdir} ]; then
      /bin/rm -f ${tmpdir}/*
   else
      mkdir -p ${tmpdir}
   ...
   for cych in "${cyc_opt[@]}"; do
      ...
      do
         ...
         case ${jfile} in
            aqm) aod_file=OBS_${aod_file_type}_${jfile}_${obssat}_${NOW}_${cych}_${qc}.nc
...
AOD_Dust=>aod   0 y,x aerosol optical depth (unitless)
t->time 0 t time
time_bounds->tb 0 t Scan start and end times in seconds since epoch (2000-01-01 12:00:00)
ENDVARS
EOF
                  ;;
            hysplit) aod_file=OBS_${aod_file_type}_${jfile}_${obssat}_${NOW}_${cych}_${qc}.nc
               if [ -e ${jfile}.ctl ]; then /bin/rm -f ${jfile}.ctl; fi
               cat >  ${jfile}.ctl <<EOF
dset ${comdir}/${aod_file}
undef  -9999.
...
EOF
                  ;;
            ngac) aod_file=OBS_${aod_file_type}_${jfile}_${obssat}_${NOW}_${cych}_${qc}.nc
                  ;;
...
'${gs_dir}/draw.${aod_type}.${jfile}.gs ${gs_dir} ${outdir} ${obssat} ${capobssat} ${NOW} ${cych} ${qc} conus'
'set lat ${lat0[${ieast}]} ${lat1[${ieast}]}'
'set lon ${lon0[${ieast}]} ${lon1[${ieast}]}'
'${gs_dir}/draw.${aod_type}.${jfile}.gs ${gs_dir} ${outdir} ${obssat} ${capobssat} ${NOW} ${cych} ${qc} east'
'set lat ${lat0[${iwest}]} ${lat1[${iwest}]}'
'set lon ${lon0[${iwest}]} ${lon1[${iwest}]}'
'${gs_dir}/draw.${aod_type}.${jfile}.gs ${gs_dir} ${outdir} ${obssat} ${capobssat} ${NOW} ${cych} ${qc} west'
'set lat ${lat0[${ine10}]} ${lat1[${ine10}]}'
'set lon ${lon0[${ine10}]} ${lon1[${ine10}]}'
'${gs_dir}/draw.${aod_type}.${jfile}.gs ${gs_dir} ${outdir} ${obssat} ${capobssat} ${NOW} ${cych} ${qc} ne'
'set lat ${lat0[${inw10}]} ${lat1[${inw10}]}'
'set lon ${lon0[${inw10}]} ${lon1[${inw10}]}'
'${gs_dir}/draw.${aod_type}.${jfile}.gs ${gs_dir} ${outdir} ${obssat} ${capobssat} ${NOW} ${cych} ${qc} nw'
'set lat ${lat0[${ise10}]} ${lat1[${ise10}]}'
'set lon ${lon0[${ise10}]} ${lon1[${ise10}]}'
'${gs_dir}/draw.${aod_type}.${jfile}.gs ${gs_dir} ${outdir} ${obssat} ${capobssat} ${NOW} ${cych} ${qc} se'
'set lat ${lat0[${iswse}]} ${lat1[${iswse}]}'
'set lon ${lon0[${iswse}]} ${lon1[${iswse}]}'
'${gs_dir}/draw.${aod_type}.${jfile}.gs ${gs_dir} ${outdir} ${obssat} ${capobssat} ${NOW} ${cych} ${qc} sw'
'set lat ${lat0[${inyc}]} ${lat1[${inyc}]}'
'set lon ${lon0[${inyc}]} ${lon1[${inyc}]}'
'${gs_dir}/draw.${aod_type}.${jfile}.gs ${gs_dir} ${outdir} ${obssat} ${capobssat} ${NOW} ${cych} ${qc} nyc'
'set lat ${lat0[${imd}]} ${lat1[${imd}]}'
'set lon ${lon0[${imd}]} ${lon1[${imd}]}'
'${gs_dir}/draw.${aod_type}.${jfile}.gs ${gs_dir} ${outdir} ${obssat} ${capobssat} ${NOW} ${cych} ${qc} md'
'set lat ${lat0[${imdatl}]} ${lat1[${imdatl}]}'
'set lon ${lon0[${imdatl}]} ${lon1[${imdatl}]}'
'${gs_dir}/draw.${aod_type}.${jfile}.gs ${gs_dir} ${outdir} ${obssat} ${capobssat} ${NOW} ${cych} ${qc} mdatl'
'quit'
EOF
...
'c'
'open ${jfile}.ctl'
'${gs_dir}/draw.${aod_type}.${jfile}.gs ${gs_dir} ${outdir} ${obssat} ${capobssat} ${NOW} ${cych} ${qc} dset'
'set lat ${lat0[${iconus}]} ${lat1[${iconus}]}'
'set lon ${lon0[${iconus}]} ${lon1[${iconus}]}'
'${gs_dir}/draw.${aod_type}.${jfile}.gs ${gs_dir} ${outdir} ${obssat} ${capobssat} ${NOW} ${cych} ${qc} conus'
'set lat ${lat0[${ieast}]} ${lat1[${ieast}]}'
...
if [ "${flag_qsub}" == "yes" ]; then
   ...
   task_cpu='05:00:00'
   job_name=cmaq_${aod_type}_${obssat}
   batch_script=trans_cmaq_${aod_type}_${obssat}.sh
...
EOF
   ...
   while [ ${NOW} -le ${LASTDAY} ]; do
      ...
      for j in "${type[@]}"; do
         data_dir=${ftpdir}/${obssat}_${aod_type}_${j}_${NOW}${i}
         if [ -d ${data_dir} ]; then
