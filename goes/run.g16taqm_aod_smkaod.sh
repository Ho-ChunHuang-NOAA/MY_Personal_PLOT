...
if [ 1 -eq 1 ]; then
            for i in "${qc_flag[@]}"; do
               ...
               out_file=${OBSOUT}/OBS_SMKAOD_${mdl_name}_${obs_name}_${NOW}_${oh}_${i}.nc
               if [ "${i}" == "99" ]; then
                  point2grid ${aod_file} ${mod_file} ${out_file} -field 'name="AOD_Smoke"; level="(*,*)";' -method UW_MEAN -v 1 -qc ${value} -adp ${adp_file}
               else
                  point2grid ${aod_file} ${mod_file} ${out_file} -field 'name="AOD_Smoke"; level="(*,*)";' -method UW_MEAN -v 1 -qc ${value} -adp ${adp_file}
               fi
               ...
               if [ ! -s ${out_file} ]; then
                  echo "WARNING : Satellite Smoke AOD regridding failed ${outfile} - point2grid"
               fi
            done
fi
         else
            echo "WARNING:  no Satellite Smoke AOD observation data at ${NOW} hour ${oh}"
         fi
         ((ohr++))
      done
      if [ "${flag_hpss_archive}" == "yes" ]; then
         ...
         hpssdir=${hpss_root}/${YY}_GOES_16_Smoke_AOD_REGRID/${YM}
         hsi mkdir  ${hpssdir}
