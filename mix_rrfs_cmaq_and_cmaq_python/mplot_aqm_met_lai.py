...
if envir == "prod":
    script_name = [
                  "daily.aqm.plot_met_v5s5.py"
                  ]
    col_var = [ "pm25_col" ]
else:
    script_name = [
                  "daily.aqm.plot_met_v6s5.py",
                  "diff.aqm.plot_met_v5_6s5.py"
                  ]
...
while date <= edate:
    for cyc in cycle:
        ...
        for i in script_name:
            msg=datetime.datetime.now()
            if i == "daily.aqm.plot_met_v5s5.py" and envir == "prod":
            ...
            if i == "daily.aqm.plot_met_v6s5.py" and envir != "prod":
                print("    Start processing "+i)
                msg="        python "+i+" "+envir+" "+cyc+" "+date.strftime(YMD_date_format)+" "+date.strftime(YMD_date_format)
                print(msg)
                subprocess.call(["python "+i+" "+envir+" "+cyc+" "+date.strftime(YMD_date_format)+" "+date.strftime(YMD_date_format)+" > "+i+"."+envir+"."+cyc+"."+date.strftime(YMD_date_format)+".log 2>&1 &"], shell=True)
            if i == "diff.aqm.plot_met_v5_6s5.py" and envir != "prod":
                print("    Start processing "+i)
