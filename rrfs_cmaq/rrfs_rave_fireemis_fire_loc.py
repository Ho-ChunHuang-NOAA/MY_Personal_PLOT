...
model="aqm"
cycle=[ "12" ]
working_dir="/gpfs/dell1/stmp/Ho-Chun.Huang/working/rrfs_fireemis/"+envir
metout="/gpfs/hps/nco/ops/com/aqm/prod/"+model+"."+grdcro2d_date
date = sdate
while date <= edate:
    find_dir=[
              "/gpfs/dell2/emc/retros/noscrub/Jianping.Huang/emissions/GSCE/GBBEPx.in.C775/Reprocessed/"+date.strftime(YMD_date_format),
              "/gpfs/dell2/emc/modeling/noscrub/Ho-Chun.Huang/RRFS_CMAQ/RAVE",
              ...
              "/gpfs/dell1/ptmp/Ho-Chun.Huang/com/aqm/"+envir+"/aqm."+date.strftime(YMD_date_format)
             ]
    flag_find_idir="no"
    for idir in find_dir:
        ...
        for cyc in cycle:
            ...
            check_file="GBBEPx_C775GRID.emissions_v003_"+date.strftime(YMD_date_format)+".nc"
            check_file="Hourly_Emissions_regrid_rrfs_13km_"+date.strftime(YMD_date_format)+"_t12z_h72.nc"
    ...
    if flag_find_idir == "yes":
        ...
        for idir in find_dir:
            ...
            for cyc in cycle:
                check_file=model+".t"+cyc+"z.fireemis.d1.ncf"
                check_file="GBBEPx_C775GRID.emissions_v003_"+date.strftime(YMD_date_format)+".nc"
                check_file="Hourly_Emissions_regrid_rrfs_13km_"+date.strftime(YMD_date_format)+"_t"+cyc+"z_h72.nc"
                aqmfilein=datadir+"/"+check_file
                if os.path.exists(aqmfilein):
                    print(aqmfilein+" exists")
                    cycfind=cyc
    ...
    figout=figdir+"/rrfs_fireemis_"+envir+"_"+date.strftime(YMD_date_format)
    ...
    for cyc in cycle:
        ...
        aqmfilein=datadir+"/GBBEPx_C775GRID.emissions_v003_"+date.strftime(YMD_date_format)+".nc"
        aqmfilein=datadir+"/Hourly_Emissions_regrid_rrfs_13km_"+date.strftime(YMD_date_format)+"_t"+cyc+"z_h72.nc"
        if os.path.exists(aqmfilein):
            print(aqmfilein+" exists")
            ...
            i=mdl_lat.shape[0]
            j=mdl_lat.shape[1]
            print('"Dimension 0 LAT is %d"' % (i))
            print('"Dimension 1 LON is %d"' % (j))
            mdl_pts=i*j
            ...
            i=fire_cs.shape[1]
            j=fire_cs.shape[2]
            print('"Dimension 0 NSTEP is %d"' % (k))
            print('"Dimension 1 LAT   is %d"' % (i))
            print('"Dimension 2 LON   is %d"' % (j))
            cs_aqm.close()
                ...
                if current_max_fire > 1.0E+20:
                    ...
                    if n == 0:
                        ...
                        if current_max_fire > max_fire:
                            ...
                            max_rows=idx_rows
                            max_cols=idx_cols
                    print("maximum fire emission at step "+str(format(n,'02d'))+" is "+str(format(current_max_fire,'02e')))
        ...
        for i in range(0,ilen):
            if int(iplot[i]) == 1: 
                ...
                ## ax.set_title(date.strftime(YMD_date_format)+" "+envir.upper()+" run fire location - "+emis_var.upper())
                ax.set_title(date.strftime(YMD_date_format)+" RAVE daily observed fire location - "+emis_var.upper())
                ## ax.add_feature(rivers_50m, facecolor='None', edgecolor='b')
                ## ax.add_feature(cfeature.RIVERS)
                ## plt.show()
                for x in range(0,num_pts):
                  ## ax.plot(lon[x], lat[x], 'ro', markersize=3, transform=ccrs.Geodetic())
                  ax.plot(cs_lon[x], cs_lat[x], 'ro', markersize=1, transform=ccrs.PlateCarree())
                ##ax.text(-117, 33, 'San Diego', transform=ccrs.Geodetic())
                ## ## fileout=figout+"/rrfs_rave."+figarea+"."+envir+"."+date.strftime(YMD_date_format)+".t12z.location.png"
                fileout=figout+"/rrfs_rave."+figarea+"."+date.strftime(YMD_date_format)+".t12z.location.png"
                plt.savefig(fileout, bbox_inches='tight') 
                ## for j in range(1,3):
                ##    newfile=figout+"/rrfs_rave."+figarea+"."+envir+"."+date.strftime(YMD_date_format)+".t12z.location.day"+str(j)+".k1.png"
        ...
        partb=os.path.join("hchuang@rzdm:", "home", "www", "emc", "htdocs", "mmb", "hchuang", "rrfs_fire_location")
