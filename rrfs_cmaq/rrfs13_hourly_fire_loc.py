    ...
    for cyc in cycle:
        ...
        if os.path.exists(aqmfilein):
            ...
            for n in range(0,nstep):
                ...
                ncol=xnonzero[1]
                nx=len(nrow)
                print('at fcst hour %d the number of fire source is %d' % ( n+1, nx ) )
                print('maximum/minimum  fire emission occurred is %f and %f' % ( max_fire, min_fire ) )
                cs_lat=[]
                cs_lon=[]
                for i in range(0,nx):
                    cs_lat.append(mdl_lat[nrow[i],ncol[i]])
                    cs_lon.append(mdl_lon[nrow[i],ncol[i]])
                ...
                for i in range(0,ilen):
                    if int(iplot[i]) == 1: 
                        ...
                        fcstz=n+1
                        ax.set_title(date.strftime(YMD_date_format)+" "+envir.upper()+" "+emission_resolution.upper()+" "+emission_version.upper()+" FCST HR "+str(fcstz)+" fire location - "+emis_var.upper())
                        ...
                        fileout=figout+"/"+envir+emission_resolution+".fireemis."+figarea+"."+date.strftime(YMD_date_format)+"."+str(fcstz)+".location.png"
