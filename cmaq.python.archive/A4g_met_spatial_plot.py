...
#'''        
        ...
        cb = plt.colorbar(sc,orientation='horizontal',pad=0.05,aspect=50,extend='both').set_label(label='T2 Bias ($\/o^C$)',size=14)
#        if daily=='ave':
#            plt.title('%sz %s-CMAQ: %s'%(run,mod,dt_str),fontsize=16)
#            cb = plt.colorbar(sc,orientation='horizontal',pad=0.05,aspect=50,extend='both').set_label(label='Daily Avg PM2.5 Bias ($\u03bcg/m^3$)',size=14)
        ...
        plt.savefig(fig_dir+'/' + d + '_%sz_T2_bias.png'%run, dpi=300, bbox_inches='tight') # save figure
        plt.close(fig2)
'''
'''
#######################################################
