...
start = date(2019,8,20) # June 1, 2019 (2019,6,1)
end = date(2019,8,20) # June 28, 2019 (2019,6,28)
...
# Change directory
d_str = base_dir+'Data/AirNow/201908_T05_T04'
os.chdir(d_str) 
...
#ax3.set_title('Time Series: August 2019 (Birmingham, AL)',fontsize='14')
ax3.set_title('Time Series: August 2019 (Spokane, WA)',fontsize='14')
...
#ax4.set_title('Time Series: August 2019 (Birmingham, AL)',fontsize='14')
ax4.set_title('Time Series: August 2019 (Spokane, WA)',fontsize='14')
...
# Iterate over files in directory 
d_str1 = base_dir+'Data/AirNow/201908_T13_T12/'
...
for v in var_all:
    ...
    plt.axhline(y=0, color='k', linestyle='-')
#    ax3.set_title('Time Series (Birmingham, AL) | Initial Time: 2019-08-20 12z',fontsize='12')
    ax3.set_title('Time Series (Spokane, WA) | Initial Time: 2019-08-20 12z',fontsize='12')
...
#########################################################################
##########     BIAS MAPS FOR DAY (HR 25) AND NIGHT (HR 36,32)     ##########
#########################################################################
...
#        for idt,dt in enumerate([alldatetimes[25-1],alldatetimes[36-1]]):    
        for idt,dt in enumerate([alldatetimes[25-1],alldatetimes[32-1]]):    
            if v=='pm25':
                lon_obs = airnow.loc[airnow['DATETIME']==dt]['LON']
                lat_obs = airnow.loc[airnow['DATETIME']==dt]['LAT']
            ...
            if idt==0:
                ax1.set_title('12z %s-CMAQ | F25 | Valid: 2019-08-21 13z'%mod,fontsize=16)
                fig1.tight_layout()
                plt.savefig(base_dir+'Output/Case_Study/%s_CMAQ_%s_bmap_f25.png'%(mod,v), dpi=300, bbox_inches='tight') # save figure
            if idt==1:
                ax1.set_title('12z %s-CMAQ | F32 | Valid: 2019-08-22 00z'%mod,fontsize=16)
                fig1.tight_layout()
                plt.savefig(base_dir+'Output/Case_Study/%s_CMAQ_%s_bmap_f32.png'%(mod,v), dpi=300, bbox_inches='tight') # save figure
...
for mod in ['NAM','FV3']:
    ...
    nc = Dataset(base_dir+'Data/%sCMAQ_3D/aqm.20190820/aqm.t12z.conc.nc'%mod,'r')
    pm25 = nc.variables['PM25_TOT'][hr_start-1:hr_end,0:20,iy,ix]    
    nc.close()
    # open and read file using netCDF function (Dataset) - June 9, 2019
    nc = Dataset(base_dir+'Data/%sCMAQ_3D/aqm.20190820/aqm.t12z.metcro3d.nc'%mod,'r')
...
#    ax2.set_title('%s-CMAQ | Initial Time: 2019-08-20 12z | Birmingham, AL'%mod,fontsize=14)
    ax2.set_title('%s-CMAQ | Initial Time: 2019-08-20 12z | Spokane, WA'%mod,fontsize=14)
...
print("The plotting with model_eval6.py is finished!")
###############################################################################
