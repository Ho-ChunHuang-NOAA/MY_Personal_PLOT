...
#hr_end = 24 # ending forecast hour (1-48)
run = '12' # model run time (00z, 06z, 12z, or 18z)
...
#        U = np.sqrt((u**2)+(v**2)) # wind speed (m/s)
        ...
        plt.savefig(fig_dir+'WindSpeed_bias.png', dpi=300, bbox_inches='tight') # save figure
...
'''        
        ...
        cb = plt.colorbar(sc,orientation='horizontal',pad=0.05,aspect=50,extend='both').set_label(label='T2 ($\/^oC$)',size=14)
#        if daily=='ave':
#            plt.title('%sz %s-CMAQ: %s'%(run,mod,dt_str),fontsize=16)
#            cb = plt.colorbar(sc,orientation='horizontal',pad=0.05,aspect=50,extend='both').set_label(label='Daily Avg PM2.5 Bias ($\u03bcg/m^3$)',size=14)
        ...
        plt.savefig(fig_dir+'/' + d + '_%sz_bias.png'%run, dpi=300, bbox_inches='tight') # save figure
        plt.close(fig2)
'''
'''
#######################################################
...
ax3.set_title('Time Series: August 2019 (CONUS)',fontsize='14')
ax3.set(xlabel='Date',ylabel='T2 ($\u03bcg/m^3$)',ylim=(4,14))
#if daily=='max':
...
# Coefficient of Determination (R^2) - FV3GFS-CMAQ
r2_fv3 = [i.at['R^2 [-]','FV3_PM'] for i in stat_pm if i.columns=='FV3_T2']
...
sc = m3.scatter(x,y,s=(abs(df_nam_all['DIFF'])*1.3)+50,c=df_nam_all['DIFF'],cmap=cmap,vmin=-10,vmax=10,norm=colors.BoundaryNorm(clevs,ncolors=cmap.N))

plt.title('12z NAM-CMAQ: August 1-31, 2019',fontsize=16)
cb = plt.colorbar(sc,orientation='horizontal',pad=0.05,aspect=50,extend='both').set_label(label='T2 Mean Bias ($\/^oC$)',size=14)
...
sc = m3.scatter(x,y,s=(abs(df_fv3_all['DIFF'])*1.0)+0,c=df_fv3_all['DIFF'],cmap=cmap,vmin=-10,vmax=10,norm=colors.BoundaryNorm(clevs,ncolors=cmap.N))
