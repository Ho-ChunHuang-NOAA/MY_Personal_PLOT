...
#base_dir = '/scratch4/NCEPDEV/naqfc/noscrub/Jianping.Huang/Python_Scripts/'
...
ax3.set_title('Time Series: August 1-31, 2019 (Southeast U.S.)',fontsize='14')
...
ax4.set_title('Time Series: August 1-31, 2019 (Southeast U.S.)',fontsize='14')
...
#d_str1 = base_dir+'Data/AirNow/201908_T05_T04/'
...
#                sc = m1.scatter(x,y,s=(abs(var_diff)*1.1)+5,c=var_diff,cmap=cmap,vmin=-50,vmax=50,norm=colors.BoundaryNorm(clevs,ncolors=cmap.N))
            ...
            if idt==0:
                ax1.set_title('12z %s-CMAQ | F25 | Valid: 2019-08-20 13z'%md,fontsize=16)
                fig1.tight_layout()
                plt.savefig(base_dir+'Output/Case_Study/%s_CMAQ_%s_bmap_f25.png'%(md,v), dpi=300, bbox_inches='tight') # save figure
            if idt==1:
                ax1.set_title('12z %s-CMAQ | F36 | Valid: 2019-08-21 00z'%md,fontsize=16)
                fig1.tight_layout()
...
for md in ['NAM','FV3']:
    ...
    nc = Dataset(base_dir+'Data/%sCMAQ_3D/aqm.20190820/aqm.t12z.conc.nc'%md,'r')
    pm25 = nc.variables['PM25_TOT'][hr_start-1:hr_end,0:20,:,:]    
    nc.close()
    # open and read file using netCDF function (Dataset) - June 9, 2019
    nc = Dataset(base_dir+'Data/%sCMAQ_3D/aqm.20190820/aqm.t12z.metcro3d.nc'%md,'r')
...
#    for k in np.arange(0,len(lat_box)):
    for k in np.arange(1,len(lat_box)):
#        print(k)
#        print(len(lat_box))
#        print(lat_box[k],lon_box[k])
        iy,ix = naive_fast(lat_cro[0,0,:,:],lon_cro[0,0,:,:], float(list(lat_box)[k]), float(list(lon_box)[k]))
...
print("The plotting with model_eval7.py is finished!")
