...
start = date(2019,8,1) # Jan 1, 2019
#end = date(2019,8,31)# Jan 30, 2019
end = date(2019,8,31)# Jan 30, 2019
run = '12' # model run time (00z, 06z, 12z, or 18z)
#daily = 'max' # PM2.5 daily 1-hr maximum ('max') or 24-hr average ('ave') 
daily = 'max' # daily 8-hr ave maximum O3 ('max') 
...
'''   
#######################################################
##########     AirNow O3 Observations     ##########
#######################################################

# Change directory
#d_str = base_dir+'Data/AirNow/201906_T05_T04'
#d_str = '/scratch2/NCEPDEV/naqfc/Jianping.Huang/Python_Scripts/Data/AirNow/201908_T05_T04'
d_str = '/scratch2/NCEPDEV/naqfc/Jianping.Huang/Python_Scripts/Data/AirNow/201908_T05_T04_O3'
os.chdir(d_str) 
...
airnow = []
colnames=['LAT', 'LON', 'DATETIME', 'PARAM', 'OZONE', 'UNITS']
usecol = ['LAT','LON','OZONE'] 
for a in airnow_obs:
    data = pd.read_csv(a,names=colnames,usecols=usecol,header=None)
    data[data['OZONE']<0]=np.nan # ignore negative O3 values
#    if daily=='max':
#        dat = data.groupby(['LAT','LON'],sort=False,as_index=False).max()
#    if daily=='ave':
#        dat = data.groupby(['LAT','LON'],sort=False,as_index=False).mean()
# Below is used to calculate daily 8-hr max ozone
    if daily=='max':
#jp        dat1 = data.groupby(['LAT','LON'],sort=False,as_index=False).tail(8).groupby(['LAT','LON'],sort=False,as_index=False).mean()
        dat1 = data.groupby(['LAT','LON'],sort=False,as_index=False).rolling(8).mean().dropna()
        dat2 =  dat1.groupby(['LAT','LON'],sort=False,as_index=False).max()
        dat =  dat2.groupby(['LAT'],sort=False,as_index=False).max()
        print(dat)
    airnow.append(dat)
...
#            f = 'aqm.t%sz.%s_24hr_pm25.148.nc' %(run,daily) # FV3GFS-CMAQ daily max PM2.5 (1/1/19)
        if daily=='max':
            f = 'aqm.t%sz.%s_8hr_o3.148.nc' %(run,daily) # FV3GFS-CMAQ daily 8hr_ave max (8/1/19)
...
#            pm_model = nc.variables['PMTF_1sigmalevel'][0,:,:]
        if daily=='max':
            o3_model = nc.variables['OZMAX8_1sigmalevel'][0,:,:]
#	    print(o3_model)
        ...
        for k in np.arange(0,len(lat_obs)):
            ...
            o3_mod.append(o3_model[iy,ix])
        
        # Calculate model/observation difference 
        o3_diff = o3_mod-o3_obs
        
        # Add lat, lon, & PM difference to dataframe (for overall bias map)
        df_bm = pd.DataFrame({'LAT':lat_obs,'LON':lon_obs,'DIFF':o3_diff})
        if mod=='NAM':
        ...
        df = pd.DataFrame({'%s_O3'%mod:o3_mod,'OBS_O3':o3_obs,'DIFF':o3_diff})
        mean_o3 = np.mean(df).to_frame(name=mod)
        df_o3.append(mean_o3)
        
        # Compute statistics 
        stats = met.stats(df,'%s_O3'%mod,'OBS_O3','ppbv')
        stat_o3.append(stats)
...
###############################################################
##########     Time Series Plots - DAILY 8-HR MAX O3    #######
###############################################################
...
# Create a time series plot of O3 averaged over CONUS domain
fig3,ax3 = plt.subplots(figsize=(6,4))

# Select AirNow observations, NAM-CMAQ output, and FV3GFS-CMAQ output
y_obs = [i.at['OBS_O3','NAM'] for i in df_o3 if i.columns=='NAM']
y_nam = [i.at['NAM_O3','NAM'] for i in df_o3 if i.columns=='NAM']
y_fv3 = [i.at['FV3_O3','FV3'] for i in df_o3 if i.columns=='FV3']
...
#ax3.set_title('Time Series: August 2019 (CONUS)',fontsize='14')
ax3.set_title('Time Series: August 2019 (CONUS)',fontsize='14')
if daily=='ave':
    ax3.set(xlabel='Date',ylabel='O3 (ppbv)',ylim=(20,60))
if daily=='max':
    ax3.set(xlabel='Date',ylabel='O3 (ppbv)',ylim=(20,70))
...
fig3.savefig(fig_dir+'O3_%s_timeseries.png'%daily,dpi=300,bbox_inches='tight')

###############################################################################

# Create Daily 8-hr Ave Max O3 NMB time series plot
fig4,ax4 = plt.subplots(figsize=(6,4))

# Normalized Mean Bias (NMB) - NAM-CMAQ
nmb_nam = [i.at['NMB [%]','NAM_O3'] for i in stat_o3 if i.columns=='NAM_O3']
ax4.plot(date_list,nmb_nam,c='b',label='NAM-CMAQ')
# Normalized Mean Bias (NMB) - FV3GFS-CMAQ
nmb_fv3 = [i.at['NMB [%]','FV3_O3'] for i in stat_o3 if i.columns=='FV3_O3']
ax4.plot(date_list,nmb_fv3,c='r',label='FV3GFS-CMAQ')

plt.axhline(y=0, color='k', linestyle='-')
#ax4.set_title('Time Series: August 2019 (CONUS)',fontsize='14')
ax4.set_title('Time Series: August 2019 (CONUS)',fontsize='14')
if daily=='max':
    ax4.set(xlabel='Date',ylabel='Daily 8-hr Max O\u2083 NMB (%)',ylim=(-40,70))
plt.xticks(rotation=60)
ax4.legend(loc='best',fancybox=True, shadow=True)
#plt.show()
fig4.tight_layout()
fig4.savefig(fig_dir+'O3_%s_timeseries_nmb.png'%daily,dpi=300,bbox_inches='tight')
 
###############################################################################

# Create Daily 8-hr max O3 NME time series plot
fig5,ax5 = plt.subplots(figsize=(6,4))

# Normalized Mean Error (NME) - NAM-CMAQ
nme_nam = [i.at['NME [%]','NAM_O3'] for i in stat_o3 if i.columns=='NAM_O3']
ax5.plot(date_list,nme_nam,c='b',label='NAM-CMAQ')
# Normalized Mean Error (NME) - FV3GFS-CMAQ
nme_fv3 = [i.at['NME [%]','FV3_O3'] for i in stat_o3 if i.columns=='FV3_O3']
ax5.plot(date_list,nme_fv3,c='r',label='FV3GFS-CMAQ')

#ax5.set_title('Time Series: August 2019 (CONUS)',fontsize='14')
ax5.set_title('Time Series: August 2019 (CONUS)',fontsize='14')
if daily=='ave':
    ax5.set(xlabel='Date',ylabel='O3 NME (%)',ylim=(20,110))
if daily=='max':
    ax5.set(xlabel='Date',ylabel='Daily 8-hr Max O\u2083 NME (%)',ylim=(20,80))
plt.xticks(rotation=60)
ax5.legend(loc='best',fancybox=True, shadow=True)
#plt.show()
fig5.tight_layout()
fig5.savefig(fig_dir+'O3_%s_timeseries_nme.png'%daily,dpi=300,bbox_inches='tight')

###############################################################################

# Create Daily 8-hr max O3 RMSE time series plot
fig6,ax6 = plt.subplots(figsize=(6,4))

# Root Mean Square Error (RMSE) - NAM-CMAQ
rmse_nam = [i.at['RMSE [ppbv]','NAM_O3'] for i in stat_o3 if i.columns=='NAM_O3']
ax6.plot(date_list,rmse_nam,c='b',label='NAM-CMAQ')
# Root Mean Square Error (RMSE) - FV3GFS-CMAQ
rmse_fv3 = [i.at['RMSE [ppbv]','FV3_O3'] for i in stat_o3 if i.columns=='FV3_O3']
ax6.plot(date_list,rmse_fv3,c='r',label='FV3GFS-CMAQ')

ax6.set_title('Time Series: August 2019 (CONUS)',fontsize='14')
if daily=='max':
    ax6.set(xlabel='Date',ylabel='RMSE of Daily 8-hr Max O\u2083 (ppbv)',ylim=(0,60))
#if daily=='max':
...
#plt.show()
fig6.tight_layout()
fig6.savefig(fig_dir+'O3_%s_timeseries_rmse.png'%daily,dpi=300,bbox_inches='tight')

###############################################################################

# Create O3 R^2 time series plot
fig7,ax7 = plt.subplots(figsize=(6,4))

# Coefficient of Determination (R^2) - FV3GFS-CMAQ
r2_nam = [i.at['R^2 [-]','NAM_O3'] for i in stat_o3 if i.columns=='NAM_O3']
ax7.plot(date_list,r2_nam,c='b',label='NAM-CMAQ')
# Coefficient of Determination (R^2) - FV3GFS-CMAQ
r2_fv3 = [i.at['R^2 [-]','FV3_O3'] for i in stat_o3 if i.columns=='FV3_O3']
ax7.plot(date_list,r2_fv3,c='r',label='FV3GFS-CMAQ')

ax7.set_title('Time Series: August 2019 (CONUS)',fontsize='14')
ax7.set(xlabel='Date',ylabel='$R^2$ Daily 8-hr Max O\u2083',ylim=(0,0.4))
plt.xticks(rotation=60)
ax7.legend(loc='best',fancybox=True, shadow=True)
#plt.show()
fig7.tight_layout()
fig7.savefig(fig_dir+'O3_%s_timeseries_r2.png'%daily,dpi=300,bbox_inches='tight')

###############################################################################

# Get average Daily 8-hr max O3 biases at each station for the entire time period
...
# Draw the contours and filled contours
#clevs = [-8,-6,-4,-2,0,2,4,6,8,10,12,14]
clevs = [-14, -12, -10,-8,-6,-4,-2,0,2,4,6,8,10,12,14]
...
plt.title('12z NAM-CMAQ: August 1-31, 2019',fontsize=16)
cb = plt.colorbar(sc,orientation='horizontal',pad=0.05,aspect=50,extend='both').set_label(label='Daily 8-hr Max O\u2083 Mean Bias (ppbv)',size=14)

#plt.show()
fig8.tight_layout()
plt.savefig(fig_dir+'Overall_Bias_Map_NAM_D8MO3.png', dpi=300, bbox_inches='tight') # save figure

###############################################################
##########     Overall Daily 8-hr Max O3 Bias Map - GFS-CMAQ     ##########
###############################################################
...
plt.title('12z GFS-CMAQ: August 1-31, 2019',fontsize=16)
cb = plt.colorbar(sc,orientation='horizontal',pad=0.05,aspect=50,extend='both').set_label(label='Daily 8-hr Max O\u2083 Mean Bias (ppbv)',size=14)

#plt.show()
fig9.tight_layout()
plt.savefig(fig_dir+'Overall_Bias_Map_FV3_D38MO3.png', dpi=300, bbox_inches='tight') # save figure
