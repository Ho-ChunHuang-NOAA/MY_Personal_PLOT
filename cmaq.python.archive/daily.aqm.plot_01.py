...
### READ data and print one at a time
### PASSED AGRUEMENTS
if len(sys.argv) < 5:
...
while date <= edate:
    for cyc in cycle:
        figdir = figout+"/aqm"+envir+date.strftime(YMD_date_format)+"_"+cyc
        if os.path.exists(figdir):
            shutil.rmtree(figdir)
        os.makedirs(figdir)
        s1_title=envir.upper()+" "+date.strftime(YMD_date_format)+" "+cyc
        fcst_ini=datetime.datetime(date.year, date.month, date.day, int(cyc[1:3]))

        metfilein=comout+"/aqm."+date.strftime(YMD_date_format)+"/aqm."+cyc+".grdcro2d.ncf"
        ...
        aqmfilein=comout+"/aqm."+date.strftime(YMD_date_format)+"/aqm."+cyc+".aconc_sfc.ncf"
        ...
        metfilein=comout+"/AK."+date.strftime(YMD_date_format)+"/aqm."+cyc+".grdcro2d.ncf"
        if os.path.exists(metfilein):
            print(metfilein+" exists")
            model_data = netcdf.Dataset(metfilein)
        ...
        metfilein=comout+"/HI."+date.strftime(YMD_date_format)+"/aqm."+cyc+".grdcro2d.ncf"
        if os.path.exists(metfilein):
            print(metfilein+" exists")
            model_data = netcdf.Dataset(metfilein)
        ...
        for ivar in range(0,num_var):
            ...
            for n in range(0,2):
                ...
                title=s1_title+"\n"+s2_title+" "+s3_title
                if flag_open_cs:
                    pvar_cs = cs_aqm.variables[var_read][n,0,:,:] * scale
                if flag_open_ak:
                    pvar_ak = ak_aqm.variables[var_read][n,0,:,:] * scale
                if flag_open_hi:
                    pvar_hi = hi_aqm.variables[var_read][n,0,:,:] * scale
                for ireg in range(0,num_reg):
                    if iplot[ireg] == 1:
                        ...
                        fig.colorbar(cf1,cmap=cmap,orientation='horizontal',pad=0.015,aspect=80,extend='both',ticks=clevs,norm=norm,shrink=0.8)
                        savefig_name = figdir+"/aqm."+figarea+"."+envir.lower()+"."+date.strftime(YMD_date_format)+"."+cyc+"."+str(format(nout,'02d'))+"."+var[ivar]+".k1.png"
