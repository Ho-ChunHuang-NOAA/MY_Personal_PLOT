...
#m = Meso(token = '90782d7315384f8b8789de7441f95ebb')
...
#        U = np.sqrt((u**2)+(v**2)) # wind speed (m/s)
        ...
        if mod=='NAM':
           plt.savefig(fig_dir+'/NAM_'+'WindSpeed_bias.png', dpi=300, bbox_inches='tight') # save figure
        if mod=='FV3':
           plt.savefig(fig_dir+'/FV3_'+'WindSpeed_bias.png', dpi=300, bbox_inches='tight') # save figure
...
variables = 'air_temp'
var_list = variables.split(',')
d_str3 = base_dir+'Data/Mesonet_201908/'
...
for mod in ['NAM','FV3']: # add 'NAM' later
#for mod in ['FV3']: # add 'NAM' later
    for idt,d in enumerate(alldates):
        #if mod=='FV3GFS': # REMOVE THIS when July data available
        #    d = re.sub('06','01',d) # AND THIS
        
        # Extract AirNow info for corresponding day
        lat_obs = obs_meso_all.loc[d,'LAT']
        lon_obs = obs_meso_all.loc[d,'LON']
        pm_obs = obs_meso_all.loc[d,'air_temp']
        
        # Change directory
        os.chdir(base_dir+'Data/'+'%sCMAQ'%mod+'/'+'aqm.'+d) 

        print(base_dir+'Data/'+'%sCMAQ'%mod+'/'+'aqm.'+d)
        
#        if daily=='max':
#            f = 'aqm.t%sz.metcro2d.nc' %(run,daily) # 
#        if daily=='ave':
#            f = 'aqm.t%sz.metcro2d.nc' %(run,daily) #
        
        # open and read file using netCDF function (Dataset)
        nc = Dataset('aqm.t%sz.metcro2d.nc'%run,'r')
        pm_model = nc.variables['TEMP2'][hr_start:hr_end+1,0,:,:] - 273.15

        # Close the netCDF file
        nc.close()
            
        lat_model = lat_cro[0,0,:,:]
        lon_model = lon_cro[0,0,:,:]

        pm_mod = []
        # Sample lat/lon grid cell information
        for k in np.arange(0,len(lat_obs)):
            iy,ix = met.naive_fast(lat_model, lon_model, lat_obs[k], lon_obs[k])
            pm_mod.append(pm_model[idt,iy,ix])
...
#'''        
        ...
        sc = m2.scatter(x,y,s=(abs(pm_diff)*1.3)+50,c=pm_diff,cmap=cmap,vmin=-10,vmax=10,norm=colors.BoundaryNorm(clevs,ncolors=cmap.N))
        plt.title('%sz %s-CMAQ: %s'%(run,mod,dt_str),fontsize=16)
        cb = plt.colorbar(sc,orientation='horizontal',pad=0.05,aspect=50,extend='both').set_label(label='T\u2082 Bias ($\/^oC$)',size=14)
#        if daily=='ave':
#            plt.title('%sz %s-CMAQ: %s'%(run,mod,dt_str),fontsize=16)
#            cb = plt.colorbar(sc,orientation='horizontal',pad=0.05,aspect=50,extend='both').set_label(label='Daily Avg PM2.5 Bias ($\u03bcg/m^3$)',size=14)
        ...
        if mod=='NAM': 
           plt.savefig(fig_dir+'/NAM'+d+'_%sz_T2_bias.png'%run, dpi=300, bbox_inches='tight') # save figure
        if mod=='FV3':
           plt.savefig(fig_dir+'/FV3'+d+'_%sz_T2_bias.png'%run, dpi=300, bbox_inches='tight') # save figure
        plt.close(fig2)
'''
'''
#######################################################
...
ax3.set_title('Time Series: August 2019 (CONUS)',fontsize='14')
ax3.set(xlabel='Date',ylabel='T2 ($\/o^C$)',ylim=(4,14))
#if daily=='max':
...
fig3.savefig(fig_dir+'T2_%s_timeseries.png'%daily,dpi=300,bbox_inches='tight')
...
ax4.set_title('Time Series: August 2019 (CONUS)',fontsize='14')
if daily=='ave':
    ax4.set(xlabel='Date',ylabel='T2 NMB (%)',ylim=(-40,70))
if daily=='max':
    ax4.set(xlabel='Date',ylabel='T2 NMB (%)',ylim=(-40,160)) 
plt.xticks(rotation=60)
ax4.legend(loc='best',fancybox=True, shadow=True)
#plt.show()
fig4.tight_layout()
fig4.savefig(fig_dir+'T2_%s_timeseries_nmb.png'%daily,dpi=300,bbox_inches='tight')
...
ax5.set_title('Time Series: August 2019 (CONUS)',fontsize='14')
if daily=='ave':
    ax5.set(xlabel='Date',ylabel='T2 NME (%)',ylim=(20,110))
if daily=='max':
    ax5.set(xlabel='Date',ylabel='T2 NME (%)',ylim=(20,180))
plt.xticks(rotation=60)
ax5.legend(loc='best',fancybox=True, shadow=True)
#plt.show()
fig5.tight_layout()
fig5.savefig(fig_dir+'T2_%s_timeseries_nme.png'%daily,dpi=300,bbox_inches='tight')
...
rmse_nam = [i.at['RMSE [$\u03bcg/m^3$]','NAM_T2'] for i in stat_pm if i.columns=='NAM_T2']
print(date_list)
print(rmse_nam)
ax6.plot(date_list,rmse_nam,c='b',label='NAM-CMAQ')
# Root Mean Square Error (RMSE) - FV3GFS-CMAQ
rmse_fv3 = [i.at['RMSE [$\u03bcg/m^3$]','FV3_PM'] for i in stat_pm if i.columns=='FV3_T2']
ax6.plot(date_list,rmse_fv3,c='r',label='FV3GFS-CMAQ')

ax6.set_title('Time Series: August 2019 (CONUS)',fontsize='14')
if daily=='ave':
    ax6.set(xlabel='Date',ylabel='T2 RMSE ($\u03bcg/m^3$)',ylim=(0,16))
if daily=='max':
    ax6.set(xlabel='Date',ylabel='T2 RMSE ($\u03bcg/m^3$)',ylim=(0,40))
plt.xticks(rotation=60)
ax6.legend(loc='best',fancybox=True, shadow=True)
#plt.show()
fig6.tight_layout()
fig6.savefig(fig_dir+'T2_%s_timeseries_rmse.png'%daily,dpi=300,bbox_inches='tight')
...
# Coefficient of Determination (R^2) - FV3GFS-CMAQ
r2_nam = [i.at['R^2 [-]','NAM_T2'] for i in stat_pm if i.columns=='NAM_T2']
ax7.plot(date_list,r2_nam,c='b',label='NAM-CMAQ')
# Coefficient of Determination (R^2) - FV3GFS-CMAQ
r2_fv3 = [i.at['R^2 [-]','FV3_T2'] for i in stat_pm if i.columns=='FV3_T2']
ax7.plot(date_list,r2_fv3,c='r',label='FV3GFS-CMAQ')

ax7.set_title('Time Series: August 2019 (CONUS)',fontsize='14')
ax7.set(xlabel='Date',ylabel='T2 $R^2$',ylim=(0,0.4))
plt.xticks(rotation=60)
ax7.legend(loc='best',fancybox=True, shadow=True)
#plt.show()
fig7.tight_layout()
fig7.savefig(fig_dir+'T2_%s_timeseries_r2.png'%daily,dpi=300,bbox_inches='tight')
'''
###############################################################################
...
sc = m3.scatter(x,y,s=(abs(df_nam_all['DIFF'])*1.)+0,c=df_nam_all['DIFF'],cmap=cmap,vmin=-10,vmax=10,norm=colors.BoundaryNorm(clevs,ncolors=cmap.N))

plt.title('12z NAM-CMAQ: August 1-31, 2019',fontsize=16)
cb = plt.colorbar(sc,orientation='horizontal',pad=0.05,aspect=50,extend='both').set_label(label='T\u2082 Mean Bias ($\/^oC$)',size=14)

#plt.show()
fig8.tight_layout()
plt.savefig(fig_dir+'Overall_Bias_Map_NAM_T2.png', dpi=300, bbox_inches='tight') # save figure
#'''
###############################################################
##########     Overall PM2.5 Bias Map - GFS-CMAQ     ##########
###############################################################
...
sc = m3.scatter(x,y,s=(abs(df_fv3_all['DIFF'])*1.3)+50,c=df_fv3_all['DIFF'],cmap=cmap,vmin=-10,vmax=10,norm=colors.BoundaryNorm(clevs,ncolors=cmap.N))
...
#plt.show()
fig9.tight_layout()
plt.savefig(fig_dir+'Overall_Bias_Map_FV3_T2.png', dpi=300, bbox_inches='tight') # save figure
