   ...
   do
      ...
      do
         ...
         while [ ${NOW} -le ${LASTDAY} ]; do
            ...
            file_type=pm25_col.k1.${figure_type}
            let m=${hourly}
            while [ ${m} -le 24 ]; do
               n=`printf %2.2d ${m}`
               num=`printf %3.3d ${k}`
               ## scp -p ${remote_user}@${remote_host}:${remote_dir}/${YR}/${NOW}/${j}/${file_hd}.${n}.${file_type} plot${num}.${figure_type}
               cp -p /gpfs/dell2/stmp/${USER}/daily_plot_pm25_col/aqm_${L}_pm_col_${NOW}06/${file_hd}.${n}.${file_type} plot${num}.${figure_type}
               let m=${m}+${hourly}
               ((k++))
            done
            cdate=$NOW"00"
         ...
         fileout=aqm.${i}.${L}.${FIRSTDAY}_${LASTDAY}.${j}.3hrly_anim.pm25_col.${figure_type}
