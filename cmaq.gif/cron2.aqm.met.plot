...
##
USAGE="USAGE : $0 Cycle_hr (default:06|12) NCO_run (default:prod|para)"
if [[ $# -lt 1 ]]; then
  echo $USAGE
  exit
fi
...
grid_id2=148
grid_id=227
## Manuelly activated using scp to RZDM
flag_ftp=no
flag_ftp=yes
...
## declare -a typ=( metcro2d metcro3d metdot3d metbdy3d grdcro2d grddot2d )
FIRSTDAY=${TODAY}
LASTDAY=${TODAY}

cyc_in=$1
declare -a cyc_opt=( 06 12 )
ichk=`printf %2.2d ${cyc_in}`
flag_run=no
for i in "${cyc_opt[@]}"; do
   if [ "${i}" == "${ichk}" ]; then flag_run=yes; fi 
done
if [ "${flag_run}" == "no" ]; then
   echo " no definition for cycle ${ichk}, program stop"
   exit
fi
cyc=${ichk}
cychr=t${cyc}z
...
declare -a typ=( metcro2d )
ntyp=${#typ[@]}

echo " Perform operation on cyc = ${cyc}  cychr = ${cychr}"
...
while [ ${NOW} -le ${LASTDAY} ]; do
   ...
   if [ ${exp} == 'prod' ]; then
      ...
      comdir=/gpfs/${phase12_id}d1/emc/meso/noscrub/${USER}/com/aqm/${exp}/aqm.${NOW}
   ...
   for jfile in "${typ[@]}"
   do
      tmpdir=/gpfs/dell2/stmp/${USER}/com2_aqm_${exp}_${jfile}.${NOW}${cyc}
      if [ -d ${tmpdir} ]; then
         /bin/rm -f ${tmpdir}/*
      else
         mkdir -p ${tmpdir}
      ...
      outdir=${ftpdir}/aqm_${exp}_${jfile}_${NOW}${cyc}
      if [ -d ${outdir} ]; then
         /bin/rm -f ${outdir}/*
      else
         mkdir -p ${outdir}
...
if [ ${flag_ftp} == 'yes' ]; then
   for jfile in "${typ[@]}"; do
      ...
      FTPFIRST=${FIRSTDAY}
      FTPLAST=${LASTDAY}
      FTPCYC=${cyc}   ## need to sure in %2.2d
      ...
      FTPTYPE=${jfile}
      FTPDATADIR=${ftpdir}
      FTPLOGDIR=/ptmpp1/${USER}/batch.logs
      rundir=/stmpp1/${USER}/daily_plot_cmaq_met
      ...
      mkdir -p ${rundir}
      ##
      JOBHDR=trans_${LASTDAY}${cychr}_cmaq_${FTPTYPE}_${exp}
      basefile=${HOME}/cronjob/batch.ftp.cmaq.met.base
      jobscript=${rundir}/job2_cmaq_${FTPTYPE}_ftp_${FTPFIRST}_${FTPLAST}.sh
      if [ -e ${jobscript} ]; then /bin/rm -f ${jobscript}; fi
         sed -e "s!JOBHDR!${JOBHDR}!" -e "s!JOBHR!${JOBHR}!" -e "s!JOBMIN!${JOBMIN}!" -e "s!FTPFIRST!${FTPFIRST}!"  -e "s!FTPLAST!${FTPLAST}!" -e "s!FTPDATADIR!${FTPDATADIR}!" -e "s!FTPLOGDIR!${FTPLOGDIR}!" -e "s!FTPCYC!${FTPCYC}!" -e "s!FTPFLAGUPDATE!${FTPFLAGUPDATE}!" -e "s!FTPFILEHD!${FTPFILEHD}!" -e "s!FTPFILEDESC!${FTPFILEDESC}!" -e "s!FTPEXP!${FTPEXP}!"  -e "s!FTPTYPE!${FTPTYPE}!" ${basefile} > ${jobscript}
