...
FIRSTDAY=$3
LASTDAY=$4
working_dir=/gpfs/dell2/ptmp/${USER}/batch.logs
mkdir -p ${working_dir}
scr=`pwd`
cd ${scr}
NOW=${FIRSTDAY}
...
##          exit ;;
##    esac
   script1=daily3.aqm.pm.plot_pm25_max
   script1=daily3.aqm.pm.plot_pm
   logfile=${working_dir}/sp_pm${envir}.${NOW}.${cycid}.log
   if [ -e ${script1} ]; then
     if [ -e ${logfile} ]; then /bin/rm -rf ${logfile}; fi
      bash ${script1} ${envir} ${cycid} ${NOW} ${NOW} > ${logfile} 2>&1 &
   else
