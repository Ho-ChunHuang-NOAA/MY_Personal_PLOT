...
declare envir=( para6dbc para6ebc )
...
working_dir=/gpfs/dell1/ptmp/${USER}/cmaq_animation
...
for i in "${fig_reg[@]}"; do
   for j in "${cyc[@]}"; do
      for L in "${envir[@]}"; do
         ...
         while [ ${NOW} -le ${LASTDAY} ]; do
            ...
            file_type=max_8hr_o3.day1.k1.${figure_type}
            icnt=`printf %2.2d ${ic}`
            scp -p ${remote_user}@${remote_host}:${remote_dir}/${YR}/${NOW}/${j}/${file_hd}.${file_type} fig_${icnt}
            ((ic++))
         ...
         convert -delay ${anim_time} -loop 0 fig*   cmaq_${i}${L}.${j}.anim
  ...
  if [[ 1 == 1 ]]; then  # for RZDM maintainence
    ...
    ## scp ${file} hchuang@rzdm:/home/people/emc/hchuang/transfer
    scp aqm* hchuang@rzdm:/home/people/emc/www/htdocs/mmb/hchuang/ftp
  fi
