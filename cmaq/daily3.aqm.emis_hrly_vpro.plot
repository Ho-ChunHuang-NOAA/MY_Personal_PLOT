...
while [ ${NOW} -le ${LASTDAY} ]; do
   ...
   if [ ${exp} == 'prod' ]; then
      ...
      comdir=/gpfs/dell2/emc/modeling/noscrub/Ho-Chun.Huang/cmaq_emiss_tmp/${exp}/aqm.${NOW}
      if [ ! -d ${comdir} ]; then
...
'set lon ${plotlon}'
'set z 1 35'
'${gs_dir}/draw2.aqm.${jfile}_p.gs2 ${gs_dir} ${outdir} ${exp} ${capexp} ${NOW} ${cychr} ${i} ${date}/${numj}00V${fcsti} ${plotlat} ${plotlon}'
'quit'
EOF
echo ${outdir}
...
'd poc'
endwhile
'draw title ${capexp} ${NOW} ${cychr} (${plotlat},${plotlon}) \FCST 1-13 Primary OC/EC (g/s)'
'printim ${outdir}/aqm.vpro.${exp}.${NOW}.${cychr}.poc.k1_13.png png x500 y800 white'
'c'
'quit'
EOF
echo ${outdir}
