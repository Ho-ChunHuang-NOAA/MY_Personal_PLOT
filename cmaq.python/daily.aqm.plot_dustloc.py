...
while date <= edate:
    ...
    for cyc in cycle:
        ...
        if envir == "prod":
            aqmfilein=comout+"/aqm."+date.strftime(YMD_date_format)+"/aqm."+cyc+".dustemis.ncf"
        else:
            aqmfilein=comout+"/aqm."+date.strftime(YMD_date_format)+"/aqm."+cyc+".dustemis.d1.ncf"
        if os.path.exists(aqmfilein):
            print(aqmfilein+" exists")
        ...
        flag_met3d="yes"
        met3dfilein=comout+"/aqm."+date.strftime(YMD_date_format)+"/aqm."+cyc+".metcro3d.ncf"
        if os.path.exists(met3dfilein):
            ...
            met3dfilein=comout+"/aqm."+date.strftime(YMD_date_format)+".met/aqm."+cyc+".metcro3d.ncf"
            if os.path.exists(met3dfilein):
                print(met3dfilein+" exists")
                cs_3d_met = netcdf.Dataset(met3dfilein)
                cs_fhgt = cs_3d_met.variables['ZF'][:,0,:,:]
                ...
                print("Can not find "+met3dfilein)
                met3dfilein="/gpfs/hps3/emc/meso/noscrub/Ho-Chun.Huang/com/aqm/"+envir+"/aqm."+date.strftime(YMD_date_format)+"/aqm."+cyc+".metcro3d.ncf"
                if os.path.exists(met3dfilein):
                    print(met3dfilein+" exists")
                    cs_3d_met = netcdf.Dataset(met3dfilein)
                    cs_fhgt = cs_3d_met.variables['ZF'][:,0,:,:]
