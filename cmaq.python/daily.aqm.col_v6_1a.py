...
### PASSED AGRUEMENTS
if len(sys.argv) < 5:
    print("you must set 5 arguments as model[prod|para|...] variabels[pm25_col|pm25c_col|all] cycle[06|12|all]  start_date end_date")
    sys.exit()
else:
    envir = sys.argv[1]
    sel_var = sys.argv[2]
...
working_dir=stmp_dir+"/"+envir+"_plot_working"
...
if sel_var == "all":
   var=[ "pm25_col", "pm25c_col" ]
elif sel_var == "pm25_col":
   var=[ "pm25_col" ]
elif sel_var == "pm25c_col":
   var=[ "pm25c_col" ]
else:
...
while date <= edate:
    ...
    for idir in find_dir:
        ...
        flag_find_cyc="yes"
        for cyc in cycle:
            check_file="aqm."+cyc+".conc_1a.ncf"
    ...
    for cyc in cycle:
        ...
        aqmfilein=comout+"/cs."+date.strftime(YMD_date_format)+"/aqm."+cyc+".conc_1a.ncf"
        if os.path.exists(aqmfilein):
            print(aqmfilein+" exists")
        ...
        if flag_ak == "yes":
            ...
            aqmfilein=comout+"/ak."+date.strftime(YMD_date_format)+"/aqm."+cyc+".conc_1a.ncf"
            if os.path.exists(aqmfilein):
                print(aqmfilein+" exists")
        ...
        if flag_hi == "yes":
            ...
            aqmfilein=comout+"/hi."+date.strftime(YMD_date_format)+"/aqm."+cyc+".conc_1a.ncf"
            if os.path.exists(aqmfilein):
                print(aqmfilein+" exists")
