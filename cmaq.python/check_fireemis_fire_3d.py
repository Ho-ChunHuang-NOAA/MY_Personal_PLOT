...
### PASSED AGRUEMENTS
if len(sys.argv) < 4:
    print("you must set 4 arguments as model_exp [prod|para1...#] cycle [06|12|all] start_date end_date in yyyymmdd")
    sys.exit()
else:
    envir = sys.argv[1]
    cyc_option = sys.argv[2]
...
msg_file=working_dir+"/msg_"+start_date+"_"+cyc_option
...
model="aqm"
cycle=[]
if cyc_option == "06" or cyc_option == "12":
   cycle.append(cyc_option)
elif cyc_option == "all":
   cycle = [ "06", "12" ]
...
while date <= edate:
    find_dir=[
              "/lfs/h2/emc/ptmp/"+user+"/com/aqm/"+envir+"/cs."+sdate.strftime(YMD_date_format),
             ]
    flag_find_idir="no"
    ...
    for cyc in cycle:
        ...
        if os.path.exists(aqmfilein):
            ...
            fire_cs = cs_aqm.variables['AECJ'][:,:,:,:]
            ntime=fire_cs.shape[0]
            nlay=fire_cs.shape[1]
            jmax=fire_cs.shape[2]
            imax=fire_cs.shape[3]
            print('fire emission array dimension is nstep = %d nlay = %d imax = %d jmax = %d' % ( ntime, nlay, imax, jmax))
            cs_aqm.close()
            nstep=len(cs_var)
            FEi58j285=[]
                ...
                for k in range(0,nlay):
                    ...
                    if 1 == 2 :
                        if current_max_fire > 1.0E+20:
                            print('maximum fire emission at step %d layer %d is %.2f' % (n, k, current_max_fire))
                            continue
                        else:
                            if k == 0:
                    ...
                    ncol=xnonzero[1]
                    nx=len(nrow)
                    print('==== number of fire emission at step %d layer %d is %d' % (n, k, nx))
                    if nx > 0:
                        if current_max_fire > 1.0E+20:
                            print('maximum fire emission at step %d layer %d is %.2f' % (n, k, current_max_fire))
                            continue
                        elif current_max_fire > max_fire:
                            max_fire=current_max_fire
                            max_rows=idx_rows
                            max_cols=idx_cols
                            max_lays=k
                    ...
                    print('==== maximum fire emission at step %d layer %d (i,j)=(%d,%d) is %.2f' % (n, k, max_cols[0], max_cols[0], current_max_fire))
            if 1 == 2:
                num_pts=len(plotrow)
                ## print('    final   number of fire at time step %d is %d' % (n, num_pts))
                ## print('    maximum fire emission occurred at %s z time step %d is %f at layer %d grid( %d , %d )ij ' % ( cyc, n, max_fire, k, max_cols[0], max_cols[0]) )
                if num_pts >= mdl_pts:
                    sys.exit()
                cs_lat=[]
                cs_lon=[]
            ...
            print('data inspection completed for %sz' % (cyc))
            ## for n in range(0,nstep):
