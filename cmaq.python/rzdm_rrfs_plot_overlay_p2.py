...
py_code=sys.argv[0]
if py_code.startswith("rzdm_rrfs_plot_overlay_"):
...
while date <= edate:
    ...
    for cyc in cycle:
        for ivar in range(0,num_var):
            figdir = figout+"/aqm"+"_"+envir.lower()+"obs_"+YMD+"_"+var[ivar]+cyc+"_p2"
            if os.path.exists(figdir):
                os.chdir(figdir)
                parta=os.path.join("/usr", "bin", "scp")
                if 1 == 1 :
                    partb=os.path.join("hchuang@rzdm:", "home", "www", "emc", "htdocs", "regional", "restricted", "aqm", "web", "fig", date.strftime(Y_date_format), YMD, cyc)
                else:
