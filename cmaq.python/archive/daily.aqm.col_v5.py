...
if len(sys.argv) < 5:
    print("you must set 5 arguments as model[prod|para|...] variabels[o3_col|pm25_col|all] cycle[06|12|all]  start_date end_date")
    sys.exit()
else:
    envir = sys.argv[1]
    sel_var = sys.argv[2]
...
while date <= edate:
    ...
    for idir in find_dir:
        ...
        flag_find_cyc="yes"
        for cyc in cycle:
            check_file="aqm."+cyc+".pm25_column.ncf"
    ...
    for cyc in cycle:
        ...
        aqmfilein=comout+"/aqm."+date.strftime(YMD_date_format)+"/aqm."+cyc+".pm25_column.ncf"
        if os.path.exists(aqmfilein):
            print(aqmfilein+" exists")
            ...
            for ivar in range(0,num_var):
                if var[ivar] == "o3_col":
                    ...
                    pm_cs = cs_aqm.variables['PM25_COL'][:,0,:,:]
            cs_aqm.close()
        else:
            print("Can not find "+aqmfilein)
            sys.exit()
        ...
        if flag_ak == "yes":
            ...
            aqmfilein=comout+"/AK."+date.strftime(YMD_date_format)+"/aqm."+cyc+".pm25_column.ncf"
            if os.path.exists(aqmfilein):
                print(aqmfilein+" exists")
                ...
                for ivar in range(0,num_var):
                    if var[ivar] == "o3_col":
                        ...
                        pm_ak = ak_aqm.variables['PM25_COL'][:,0,:,:]
                ak_aqm.close()
            else:
                print("Can not find "+aqmfilein)
                flag_ak = "no"
        ...
        if flag_hi == "yes":
            ...
            aqmfilein=comout+"/HI."+date.strftime(YMD_date_format)+"/aqm."+cyc+".pm25_column.ncf"
            if os.path.exists(aqmfilein):
                print(aqmfilein+" exists")
                ...
                for ivar in range(0,num_var):
                    if var[ivar] == "o3_col":
                        ...
                        pm_hi = hi_aqm.variables['PM25_COL'][:,0,:,:]
                hi_aqm.close()
            else:
                print("Can not find "+aqmfilein)
                flag_hi = "no"
