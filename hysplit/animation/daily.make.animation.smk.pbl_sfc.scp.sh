...
while [ ${NOW} -le ${LASTDAY} ]; do
   ...
   #  Get gif from RZDM
   #
   if [ "${flag_fect_gif}" == "yes" ]; then
      for i in "${fig_reg[@]}"
      do
         scp -p hchuang@rzdm:/home/www/emc/htdocs/mmb/hchuang/web/fig/${YR}/${NOW}/t06z/hysplit${i}smoke*.gif .
      done
   fi
   ...
   cd ${odir}
   scp -p $hysplit_*_${NOW}.anim.*.gif hchuang@rzdm:/home/people/emc/hchuang/transfer
