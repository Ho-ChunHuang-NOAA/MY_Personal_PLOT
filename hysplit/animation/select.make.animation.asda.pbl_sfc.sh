   ...
   do
      ...
      do
         ...
         while [ ${k} -le ${end_hour} ]; do
            num=`printf %2.2d ${k}`
            cp -p ${idir}/${NOW}/nesdis${i}smoke${j}_${num}.gif .
            ((k++))
         done
         ## convert -cache 1GB -delay ${anim_time} -loop 0 nesdis${i}smoke${j}_*.gif nesdis_${i}_$NOW.anim.${j}
         anim_file=hysplit_${i}_$NOW.anim.${j}.asda.${beg_hour}_${end_hour}.gif
         convert -delay ${anim_time} -loop 0 nesdis${i}smoke${j}_*.gif ${anim_file}
         mv ${anim_file} ${odir}
      done
   done
