...
## special setting
if [ ${exp2} == 'prod' ]; then
   mydir=/gpfs/dell2/emc/modeling/noscrub/${USER}/${smlaqm}/${exp2}
   mydir2=${mydir}
...
while [ ${NOW} -le ${LASTDAY} ]; do
   ...
   for k in "${cyc_hour[@]}"; do
      ...
      data_dir=/gpfs/dell1/stmp/${USER}/diff_${aero}_${exp2}m${exp1}/${aqm}_${area}.${NOW}.${cycle}
      if [ -d ${data_dir} ]; then
         /bin/rm -f ${data_dir}/*
      else
         mkdir -p ${data_dir}
...
ENDVARS
EOF
         gribmap -i ${data_dir}/s1_${aero}${area}.ctl
         gribmap -i ${data_dir}/s2_${aero}${area}.ctl
      ##
         t0=1
         t1=48
         let numi=t0
...
source ~/.bashrc
   ...
   while [ ${NOW} -le ${LASTDAY} ]; do
      ...
      for i in "${cyc[@]}"; do
         cycle=t${i}z
         ## data_dir=/gpfs/dell1/stmp/${USER}/${aero}_${exp}/${aqm}_${area}.${NOW}.${cycle}
